...
int getTransform(pcl::PointCloud<pcl::PointXYZ>::Ptr &src, pcl::PointCloud<pcl::PointXYZ>::Ptr &tar, double resolution,
                 pcl::Correspondences &correspondences, Eigen::Matrix4f &transform)
{
    ...
    for (int i = 0; i < 10000; ++i)
    {
        ...
        for (int k = 0; k < 3; ++k)
        {
            mse += pcl::squaredEuclideanDistance(src_transformed
                                                     .points[k],
                                                 tar_temp.points[k]);
        }
        mse /= 3;
